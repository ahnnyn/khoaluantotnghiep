...
const Login = () => {
  ...
  const handleLogin = async ({ username, password }) => {
    ...
    try {
      ...
      const { user, accessToken: token, message } = res.data;

      if (!user || !token) throw new Error(message || "Thông tin không đúng");
      ...
      if (redirectMap[user.role]) navigate(redirectMap[user.role]);
      else {
